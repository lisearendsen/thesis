...
lemma prop40rtl :
  assumes "\<exists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> set (map action p) \<subseteq> A
     \<and> insert s (set (map target p)) \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e" (*maybe make definition for alloccurs*)
  and "\<not> dependvari f M 0"
  and "\<not> dependvari g M 0"
  and "finite A"
  shows "s \<in> \<lbrakk>mu (and' g (or f (Diamond (Acts A) (var 0))))\<rbrakk> (M :: ('a, 's) lts) e"
  apply (simp add: assms(4))
  apply (rule allI)
proof
  fix S'
  from assms(1) obtain p s' where assum1 : "validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> set (map action p) \<subseteq> A
     \<and> insert s (set (map target p)) \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e" by auto
  ...
  have inS' : "validfinpath M s p s' \<and> set (map action p) \<subseteq> A
  ...
  proof
    ...
    have "source t \<in> \<lbrakk>shiftdown g 0\<rbrakk> M e" using assum4 by auto
    hence c2 : "source t \<in> \<lbrakk>g\<rbrakk> M (newenvironment e S')" using notdependshiftdown assms(3) by metis
  ...
  hence "s' \<in> \<lbrakk>g\<rbrakk> M (newenvironment e S') \<and> s' \<in> \<lbrakk>f\<rbrakk> M (newenvironment e S')" by (simp add: assms(2) assms(3) notdependshiftdown)
...
lemma prop40ltrinbetween :
  ...
  (\<exists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> (action ` set p \<subseteq> A)
     \<and> s \<in> (\<lbrakk>shiftdown g 0\<rbrakk> M e)
     \<and> target ` set p \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e)"
...
proof
  ...
  assume assum1 : "\<And>s. s \<in> ?S'
    \<longrightarrow> (\<exists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> action ` set p \<subseteq> A \<and> s \<in> \<lbrakk>shiftdown g 0\<rbrakk> M e \<and> target ` set p \<subseteq> (\<lbrakk>shiftdown g 0\<rbrakk> M e))"
  ...
  moreover {
    ...
    from this obtain s' a where assum3 : "a \<in> A \<and> (s, a, s') \<in> transition M \<and> s' \<in> ?S'" by auto
    ...
    from this obtain p s'' where tail : "validfinpath M s' p s'' \<and> s'' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> action ` set p \<subseteq> A
  ...
  ultimately show "\<exists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> action ` set p \<subseteq> A
      \<and> s \<in> \<lbrakk>shiftdown g 0\<rbrakk> M e \<and> target ` set p \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e" using splitand by blast
qed
...
lemma monotonicityformula40 : 
  ...
  by (rule monotonicitynotdependcoro(1); simp add: assms notdepends)
...
lemma prop40ltr :
  ...
  shows "\<exists>p s'. validfinpath M s p s' 
        ...
        \<and> (insert s (set (map target p)) \<subseteq> (\<lbrakk>shiftdown g 0\<rbrakk> M e))"
proof-
  ...
  hence "\<exists>p s'. validfinpath M s p s' 
        \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e
        \<and> action ` set p \<subseteq> A
        \<and> s \<in> \<lbrakk>shiftdown g 0\<rbrakk> M e
        \<and> target ` set p \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e" using prop40ltrinbetween assms(2) assms(3) assms(5) by metis
  thus ?thesis by simp
qed
...
lemma prop40 :
  ...
  shows "(s \<in> formulasemantics (mu (and' g (or f (Diamond (Acts A) (var 0))))) (M :: ('a, 's) lts) e) = 
    (\<exists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> (set (map action p) \<subseteq> A) 
      \<and> (insert s (set (map target p)) \<subseteq> \<lbrakk>shiftdown g 0\<rbrakk> M e))"
  apply (rule iffI)
...
lemma invariantApath : 
  ...
  and "s \<in> S' \<inter> {s. \<nexists>p s'. validfinpath M s p s' \<and> s' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> (set (map action p) \<subseteq> A)}"
  shows "\<exists>s' act. act \<in> A \<and> (s, act, s') \<in> transition M \<and> s' \<in> S' \<inter> {s'. \<nexists>p s''. validfinpath M s' p s'' \<and> s'' \<in> \<lbrakk>shiftdown f 0\<rbrakk> M e \<and> (set (map action p) \<subseteq> A)}"
proof-
  ...
  proof-
    ...
    hence "validfinpath M s [] s \<and> s \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action []) \<subseteq> A)" by auto
  ...
  proof-
    ...
    hence "validfinpath M s ((s, act, s')#p) s'' \<and> s'' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action ((s, act, s')#p)) \<subseteq> A)" using assum1 by simp
    thus False using assms(3) by blast
  qed
...
lemma theorem21generalizedltr : 
  ...
  and "finite A"
  shows "((\<exists>p s'. validfinpath M s p s' \<and> s' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action p) \<subseteq> A)) \<or>
...
proof-
  ...
  have "(\<nexists>p s'. validfinpath M s p s' \<and> s' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action p) \<subseteq> A)) \<Longrightarrow>  (\<exists>p. validinfpath M s p \<and> (\<forall>n. action (p n) \<in> A))"
  ...
  proof-
    ...
    let ?S' = "S' \<inter> {s'. \<nexists>p s''. validfinpath M s' p s'' \<and> s'' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action p) \<subseteq> A)}" 
  ...
  qed
  thus ?thesis by (simp only: image_subset_iff; auto)
qed

lemma theorem21generalizedrtl : 
  assumes "\<not> dependvari f M 0"
  and "finite A"
  and "(\<exists>p s'. validfinpath M s p s' \<and> s' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action p) \<subseteq> A)) \<or>
     (\<exists>p. validinfpath M s p \<and> image action (range p) \<subseteq> A)" 
  ...
  apply (rule exI)
proof
  let ?S' = "{s. (\<exists>p s'. validfinpath M s p s' \<and> s' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e) \<and> (set (map action p) \<subseteq> A)) \<or>
              (\<exists>p. validinfpath M s p \<and>  image action (range p) \<subseteq> A)}"
...
lemma theorem21generalized :  
  ...
  ((\<exists>p s'. validfinpath M s p s' \<and> (set (map action p) \<subseteq> A) \<and> s' \<in> (\<lbrakk>shiftdown f 0\<rbrakk> M e)) \<or> 
  (\<exists>p. validinfpath M s p \<and> image action (range p) \<subseteq> A))"
  apply (rule iffI)
...
lemma inductionstepmatch :
  ...
  shows "(\<exists>p s'. validfinpath M s p s' \<and> match (Star R) p \<and> s' \<in> \<lbrakk>f\<rbrakk> M e) \<Longrightarrow> (s \<in> formulasemantics (Diamond (Star R) f) M e)"
  apply (simp add: matchstar_eq_matchntimes)
  apply (rule allI)
proof
  fix S'
  ...
  proof-
    ...
    hence "\<exists>p' p'' s''. s' \<in> \<lbrakk>f\<rbrakk> M e \<and>  match R p' \<and> matchntimes n R p'' \<and> validfinpath M s p' s'' \<and> validfinpath M s'' p'' s'" using validfinpathsplit by metis
...
(*inductionstepmatch should be in this proof*)
lemma Diamondmatch :
  ...
  apply simp
  defer
  apply (rule iffI; (rule inductionstepmatch; simp)?)
proof-
  fix R Q f s e
...
next
  ...
  assume assum2 : "s \<in> \<lbrakk>Diamond (Star R) f\<rbrakk> M e"
  ...
  thus "\<exists>p s'. validfinpath M s p s' \<and> match (Star R) p \<and> s' \<in> \<lbrakk>f\<rbrakk> M e" using assum2 by (simp add: matchstar_eq_matchntimes; auto)
qed
...
proof-
  ...
  proof
    ...
    have res1 : "\<forall>s'. ?A s' = ?B s'"
      apply (subst theorem21generalized; simp add: assms notoccursnotdepends Boxcomplement_locked Diamond_enabled del: Diamond.simps Box.simps Diamond_eq_exist Box_eq_forall occurs.simps)
      apply auto
      done
...
lemma lemma50 : 
  ...
  apply (subst notdependshiftdown; simp add: assms enabled_def enabledactions_def)
  apply meson
  done 
...
next
  ...
  from this obtain j s' where IH: "j < length (succ s') \<and>  s' \<in> S' \<and> suffix i (recursiveconcpaths succ s) = conc (drop j (succ s')) (recursiveconcpaths succ (laststate (succ s')))" by auto
...
lemma feasiblesublemma (*canalwaysconstructfairpath*):
  ...
  and feasible: "\<forall>s \<in> S'. \<forall>a \<in> -B. s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> (\<exists>p s'. validfinpath M s p s' \<and> s' \<in> S' \<and> \<not> occurs \<alpha>\<^sub>f (fin p) \<and> (occurs (\<alpha>_el a) (fin p) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)))"
  and "finite (-B)"
...
proof-
  ...
  proof
    ...
    next
      ...
      from this obtain a L' where assum2: "L = insert a L' \<and> a \<notin> L' \<and> card L' = n \<and> L \<subseteq> ?L" using card_eq_SucD by metis
      from this obtain p s' where assum3: "(L' \<noteq> {} \<longrightarrow> p \<noteq> []) \<and> validfinpath M s p s' \<and> s' \<in> S' \<and> \<not> occurs \<alpha>\<^sub>f (fin p) \<and> (\<forall>a. a \<in> L' \<longrightarrow>  occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p) \<or> occurs (\<alpha>_el a) (fin p))" using IH by blast
      ...
      moreover {
        ...
        (*if we have that B is not the entire set, (which it probably never is), we do not need exclusive?*)
      ...
      moreover {
        ...
        ultimately obtain p' s'' where "validfinpath M s' p' s'' \<and> s'' \<in> S' \<and> \<not> occurs \<alpha>\<^sub>f (fin p') \<and> ((occurs (\<alpha>_el a) (fin p') \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s' (fin p')))" using persistent assum3 feasible by blast
        moreover from this have "p' \<noteq> []" using ainBcomp assum3 assum4 exclusive notemptypath persistent by metis
        ...
        moreover from this have "\<forall>a. a \<in> L' \<longrightarrow> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin (p @ p')) \<or> occurs (\<alpha>_el a) (fin (p @ p'))" by auto
...
proof-
  ...
  proof
    ...
    hence "\<exists>p s'. p \<noteq> [] \<and> validfinpath M s p s' \<and> s' \<in> S' \<and> \<not>occurs \<alpha>\<^sub>f (fin p) \<and> (\<forall>a. a \<in> -B \<and> s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p) \<or> occurs (\<alpha>_el a) (fin p))" 
      ...
      using  exclusive persistent feasible assms(4) assms(5) apply blast+
      
      done
    from this obtain p s' where "p \<noteq> [] \<and> validfinpath M s p s' \<and> s' \<in> S' \<and> \<not>occurs \<alpha>\<^sub>f (fin p) \<and> (\<forall>a. a \<in> -B \<and> s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p) \<or> occurs (\<alpha>_el a) (fin p))" by auto
    thus "?P s" using validfinpathlaststate by metis
  qed
  from this obtain succ where assum2: "\<forall>s' \<in> S'. succ s' \<noteq> [] \<and> validfinpath M s' (succ s') (laststate (succ s')) \<and> laststate (succ s') \<in> S' \<and> \<not>occurs \<alpha>\<^sub>f (fin (succ s')) \<and> (\<forall>a. a \<in> -B \<and> s' \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s' (fin (succ s')) \<or> occurs (\<alpha>_el a) (fin (succ s')))" by metis
  ...
  proof
    ...
    assume "source (?p i) \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e"
   (*opschonen*)
    ...
    { 
      ...
      ultimately have "laststate (succ s') \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e" using persistent ainBcomp by metis
...
lemma theorem24_subformula :
  ...
  and locking: "\<forall>s. locked M B s = (\<forall> a \<in> -B. s \<notin> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e)"
...
proof
  ...
  proof
    ...
    from this obtain p s' where validpath: "validfinpath M s p s' \<and> \<not> occurs \<alpha>\<^sub>f (fin p) \<and> (enabled M s' \<alpha>\<^sub>e \<or> (s' \<in> S' \<and> s' \<in> \<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) \<or> (\<exists>s'' a'. a' \<in> \<alpha>_el a \<and> a' \<notin> \<alpha>\<^sub>f \<and> (s', a', s'') \<in> transition M \<and> s'' \<in> S'))" using assum1 by auto
    ...
    moreover {
      ...
      from this obtain a' s'' where s''inS': "a' \<in> \<alpha>_el a \<and> a' \<notin> \<alpha>\<^sub>f \<and> (s', a', s'') \<in> transition M \<and> s'' \<in> S'" by auto
      ...
      moreover {
        ...
        moreover {
          ...
          moreover have "occurs (\<alpha>_el a) (fin p) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)"
            ...
            using locking persistent assum2 assum4 apply blast+
            done
          ...
          moreover have "validfinpath M s p s' \<and> \<not> occurs \<alpha>\<^sub>f (fin p) \<and> (occurs (\<alpha>_el a) (fin p) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)) \<longrightarrow> (\<exists>p s'. validfinpath M s p s' \<and> \<not> occurs \<alpha>\<^sub>f (fin p)  \<and> ((s' \<in> ?S' \<inter> (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e)) \<or> (\<exists>act s''. (s', act, s'') \<in> transition M \<and> act \<in> \<alpha>_el a \<and> act \<notin> \<alpha>\<^sub>f \<and> s'' \<in> ?S')))"
            ...
            apply (metis (no_types, lifting) Int_Collect Int_commute assum2 assum4 locking
                occursempty occursstateempty persistent validfinpath.simps(1)) (*opschonen*)
            done
...
proof-
  ...
  proof
    ...
    assume assum3 : "source (conc p p' i) \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e"
    {
      ...
      hence "occurs (\<alpha>_el a) (inf (conc (drop i p) p')) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (source (p!i)) (inf (conc (drop i p) p')) \<or> occurs (\<alpha>_el a) (inf (suffix 0 p')) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (laststate (drop i p)) (inf (suffix 0 p'))"
        using assum2 extendfinemptypath persistent occursempty occursstateleft predPentirepath validfinpath.simps(1) by metis
      hence "occurs (\<alpha>_el a) (inf (conc (drop i p) p')) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (source (p!i)) (inf (conc (drop i p) p')) \<or> occurs (\<alpha>_el a) (inf p') \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (laststate (drop i p)) (inf p')" by auto
      hence "occurs (\<alpha>_el a) (inf (conc (drop i p) p')) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (source (p!i)) (inf (conc (drop i p) p'))" using extendfinpath.simps(2) nonempty occursright occursstateright by metis
...
lemma occursemptysetfin : "occurs {} (fin p) = False"
  by simp

lemma occursemptysetinf : "occurs {} (inf p) = False"
  by simp

lemma predPsubpathsuperpathfin :
  assumes invarfin: "\<forall>p s s'. validfinpath M s p s' \<longrightarrow> locked M B s' = P s (fin p)"
...
lemma predPsubpathsuperpathinf :
  ...
  using invarinf persistent apply auto
  done

lemma predPsubpathsuperpath :
  assumes invarfin: "\<forall>p s s'. validfinpath M s p s' \<longrightarrow> P s (fin p) = locked M B s'"
  and invarinf: "\<forall>p s. validinfpath M s p \<longrightarrow> (P s (inf p) =  (\<forall>a \<in> -B. ((\<forall>i. (source (p i)) \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> (occurs (\<alpha>_el a) (inf (suffix i p)) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (source (p i)) (inf (suffix i p)))))))"
  and persistent: "\<forall>s p s' a. a \<in> -B \<and> validfinpath M s p s' \<and> (\<not>occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)  \<and> \<not>occurs (\<alpha>_el a) (fin p)) \<longrightarrow> s' \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e"
...
proof-
  assume assum1: "validfinpath M s p s' \<and> validpath M s' p'"
  {
    ...
    from this obtain s'' where "validfinpath M s p s' \<and> validfinpath M s' fp s''" using assum1 by auto
  ...
  moreover {
    ...
    hence "P s' (inf ip) = P s (inf (conc p ip))" by (subst predPsubpathsuperpathinf[where ?\<phi>_on=\<phi>_on and ?\<phi>_off=\<phi>_off and ?\<alpha>_el=\<alpha>_el and ?e=e and ?B=B]; auto simp: invarinf persistent)
    hence "P s' p' = P s (extendfinpath p p')" using assum2 by simp
  }
...
lemma notlocked : "s \<in> {s. \<nexists>p s'. validfinpath M s p s' \<and> locked M B s'} \<and> validfinpath M s p s' \<Longrightarrow> s' \<in> {s. \<nexists>p s'. validfinpath M s p s' \<and> locked M B s'}"
  apply simp
  using validfinpathsplit apply meson
  done
...
lemma feasible : 
  ...
  and exclusive: "\<forall>s. \<forall>a \<in> -B. s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> s \<notin> \<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e " 
  and persistent: "\<forall>s p s' a. a \<in> -B \<and> validfinpath M s p s' \<and> (\<not>occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)  \<and> \<not>occurs (\<alpha>_el a) (fin p)) \<longrightarrow> s' \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e"
  and feasiblesub: "\<forall>a \<in> -B. \<forall>s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e. (\<exists>p s'. validfinpath M s p s' \<and> (occurs (\<alpha>_el a) (fin p) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)))"
  and "finite (-B)"
...
lemma theorem24 :
  ...
  and "\<forall>a. \<not>dependvari (\<phi>_off a) M 0"
  and "\<forall>a. \<not>dependvari (\<phi>_on a) M 0"
  and invarfin: "\<forall>p s s'. validfinpath M s p s' \<longrightarrow>  P s (fin p) = locked M B s'"
  and invarinf: "\<forall>p s. validinfpath M s p \<longrightarrow> P s (inf p) =  ((\<forall>a \<in> -B. ((\<forall>i. (source (p i)) \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> (occurs (\<alpha>_el a) (inf (suffix i p)) \<or> occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) (source (p i)) (inf (suffix i p)))))))"
  and locking: "\<forall>s. locked M B s = (\<forall> a \<in> -B. s \<notin> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e)"
  and exclusive: "\<forall>s. \<forall>a \<in> -B. s \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e \<longrightarrow> s \<notin> \<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e " 
  and persistent: "\<forall>s p s' a. a \<in> -B \<and> validfinpath M s p s' \<and> (\<not>occursstate (\<lbrakk>shiftdown (\<phi>_off a) 0\<rbrakk> M e) s (fin p)  \<and> \<not>occurs (\<alpha>_el a) (fin p)) \<longrightarrow> s' \<in> \<lbrakk>shiftdown (\<phi>_on a) 0\<rbrakk> M e"
...
proof-
  have "\<forall>s. (s \<in> \<lbrakk>Diamond \<rho> (nu (And (-B) (\<lambda>a. or (neg (\<phi>_on a)) (Diamond (Star (Acts (-\<alpha>\<^sub>f))) (or (or (Diamond (Acts \<alpha>\<^sub>e) tt) (and' (\<phi>_off a) (var 0))) (Diamond (Acts ((\<alpha>_el a) -\<alpha>\<^sub>f)) (var 0)))))))\<rbrakk> M e =
  (\<exists>p. violating p \<rho> \<alpha>\<^sub>f \<alpha>\<^sub>e \<and>  progressing M s B p \<and> P s p))"
    apply (subst Diamondmatch)
    apply (subst theorem24_subformula; (rule assms)?)
    apply (subst splitviolating_predicate; (rule allI)+)
  proof
    fix s p p' s'
    show "validfinpath M s p s' \<and> validpath M s' p' \<Longrightarrow> P s' p' = P s (extendfinpath p p')"
      by (subst predPsubpathsuperpath [where ?\<phi>_on=\<phi>_on and ?\<phi>_off=\<phi>_off and ?\<alpha>_el=\<alpha>_el and ?e=e and ?B=B and ?M=M and ?s=s and ?p = p]; (rule assms)?; simp)
  next
    fix s
    ...
    \<or> (\<exists>p. validinfpath M s p \<and> \<not> occurs \<alpha>\<^sub>f (inf p) \<and> P s (inf p)))"
      ...
      apply (subst feasible [where ?\<phi>_on=\<phi>_on and ?\<phi>_off=\<phi>_off and ?\<alpha>_el=\<alpha>_el and ?e=e and ?B=B])
      using invarinf locking exclusive persistent feasiblesub assms(1) apply blast+
    ...
    thus "(\<exists>p s'. validfinpath M s p s' \<and> match \<rho> p \<and> ((\<exists>p s''. validfinpath M s' p s'' \<and> \<not> occurs \<alpha>\<^sub>f (fin p) \<and> (locked M B s'' \<or> enabled M s'' \<alpha>\<^sub>e)) \<or> (\<exists>p. validinfpath M s' p \<and> \<not> occurs \<alpha>\<^sub>f (inf p) \<and> P s' (inf p)))) = 
      (\<exists>p p' s'. match \<rho> p \<and> validfinpath M s p s' \<and> freeuntiloccurrence p' \<alpha>\<^sub>f \<alpha>\<^sub>e \<and> progressing M s' B p' \<and> P s' p')" by blast
  qed  
...
lemma corollary25 :
  ...
  apply (simp add: assms)
  apply (simp add: notoccursnotdepends)
  apply (simp add: notoccursnotdepends)
  apply (simp add: WFAinvarfin)
  apply (rule allI impI)+ apply (simp only: WFAinvarinf)
  apply (simp add: locked_def enabledactions_def; blast)
  apply simp
proof-
  show "\<forall>a\<in>- B. \<forall>s\<in>\<lbrakk>shiftdown (diamond a tt) 0\<rbrakk> M e. \<exists>p s'. validfinpath M s p s' \<and> (occurs {a} (fin p) \<or> occursstate (\<lbrakk>shiftdown (box a ff) 0\<rbrakk> M e) s (fin p))"
    apply (rule ballI)+
  proof-
    fix a s
    assume "s \<in> \<lbrakk>shiftdown (diamond a tt) 0\<rbrakk> M e"
  ...
  qed
next
  show "\<forall>s p s' a. a \<in> - B \<and> validfinpath M s p s' \<and> \<not> occursstate (\<lbrakk>shiftdown (box a ff) 0\<rbrakk> M e) s (fin p) \<and> \<not> occurs {a} (fin p) \<longrightarrow> s' \<in> \<lbrakk>shiftdown (diamond a tt) 0\<rbrakk> M e"
    apply (rule allI impI)+
  proof-
    fix s p s' a
    assume "a \<in> - B \<and> validfinpath M s p s' \<and> \<not> occursstate (\<lbrakk>shiftdown (box a ff) 0\<rbrakk> M e) s (fin p) \<and> \<not> occurs {a} (fin p)"
